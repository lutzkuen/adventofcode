...
class Blocksim:
        ...
        with open("input.txt", "r") as f:
            for line in f.readlines():
                ...
                for box in line.replace('\n', ''):
                    if box == '#':
                        self.active_cubes.append({'x': x, 'y': y, 'z': z})
                    x += 1
                y += 1
    ...
    def get_box_neighbors(self, x, y, z):
        neigs = 0
        is_occupied = False
        for box in self.active_cubes:
            ...
            if abs(z - box['z']) > 1:
                continue
            if abs(box['x'] - x) < 0.5 and abs(box['y'] - y) < 0.5 and abs(box['z'] - z) < 0.5:
                is_occupied = True
            # else:
